...
void start() //Запуск режиму
{
  ...
  case 6:
    second_game(true);
    break;}
    transition(led_color, black, 150);//Після закінчення режиму встановити колір на чорний
}
...
void first_game(bool multi_color_mode) // повтори колір який показується на світлодіоді(bool відповідає за те чи може колір бути змішаний з інших кольорів(тобто потребує натисканя двух кнопок))
{
  while (true)
  {
    unsigned long start_time = millis();
...
void second_game(bool multi_color_mode) //розвиток памяті, показується послідовність кольорів і користувач повинен повторити її. З кожним успіхом кількість кольорів збільшується
{
